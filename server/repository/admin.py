...
class AdminRepository(BaseRepository):
    ...
    def get_admin_by_id(self, id: int):
        admin = self.session.query(admin).filter(admin.id == id).first()
        return admin

    def update_admin(self, id: int, admin: Admin):
        admin_in_db = self.session.query(admin).filter(admin.id == id).first()
        setattr(admin_in_db, 'login', admin.login)
        setattr(admin_in_db, 'password', admin.password)
        self.session.commit()
